...
function openEditPage(project: DevAddressProject, page: DevPage): void {
  ...
  pageWikiUrl.value = page.wikiUrl || ''
  pageError.value = null
  pageEditorOpen.value = true
}
...
async function savePage(): Promise<void> {
  ...
  try {
    ...
    const wikiUrl = normalizeDevWikiUrl(pageWikiUrl.value)
...
async function openPageWiki(page: DevPage): Promise<void> {
  if (!page.wikiUrl) return
  try {
    await chrome.tabs.create({ url: page.wikiUrl })
  } catch (error) {
            ...
            <div v-if="selectedProject.pages.length" class="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
              <div class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                >
                  ...
                  <button v-if="page.wikiUrl" class="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700 transition-colors hover:bg-amber-100 dark:border-amber-900/70 dark:bg-amber-950/40 dark:text-amber-300" title="打开 Wiki" @click="openPageWiki(page)"><BookOpen :size="13" />Wiki</button>
      ...
      >
        <section class="w-full max-w-lg rounded-2xl bg-white shadow-2xl dark:bg-slate-900">
          ...
          <div class="space-y-4 p-5">
            ...
            <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300">Wiki 地址（可选）<input v-model="pageWikiUrl" class="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none ring-sky-500/30 focus:border-sky-400 focus:bg-white focus:ring-2 dark:border-slate-700 dark:bg-slate-950" placeholder="https://wiki.example.com/page" /><span class="mt-1.5 block text-[11px] font-normal text-slate-400">填写后列表行会出现 Wiki 按钮，点击可在新标签页打开。</span></label>
